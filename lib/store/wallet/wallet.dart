...
abstract class WalletBase with Store {
    ...
    Future<void> initializeRpc() async {
        this.rpcClient = RPCClient(rpcAddress: await sl.get<SharedPrefsUtil>().getRpcUrl());
    }
    ...
    Future<dynamic> getBalanceAndInsertBorrowed() async {
        if (borrowedAccount != null
            && !this.walletAccounts.contains(borrowedAccount.account)) {
                GetAccountRequest request  = GetAccountRequest(account: borrowedAccount.account.account);
                RPCResponse resp = await rpcClient.makeRpcRequest(request);
